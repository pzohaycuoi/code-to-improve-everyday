...
def check_path_exist(path, avoid_duplicate: bool = False):
    ...
    """
    workspace_dir = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
    if workspace_dir not in path: real_path = convert_to_realpath(path)
    else: real_path = path
    if os.path.exists(real_path):
        if avoid_duplicate is True: return generate_file_name(real_path)
        elif avoid_duplicate is False: return True
    else:
        if avoid_duplicate is True: return real_path
        elif avoid_duplicate is False: return False
